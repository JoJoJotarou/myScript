...
// prettier-ignore
function getUserInfo(cookie){const eventName='【用户信息】';const option={url:'https://wq.jd.com/user/info/QueryJDUserInfo?g_login_type=1&sceneval=2',headers:{Accept:'*/*','Accept-Encoding':'gzip, deflate, br','Accept-Language':'zh-cn',Host:'wq.jd.com',Cookie:cookie,Connection:'keep-alive',Referer:'https://wqs.jd.com/','User-Agent':'Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.1.1 Mobile/15E148 Safari/604.1',},};return new Promise((resolve,reject)=>{$.get(option,(err,resp,data)=>{try{if(resp.statusCode===200&&JSON.parse(data).retcode===0&&JSON.parse(data).base){const nickname=JSON.parse(data).base.nickname;const totalBeans=JSON.parse(data).base.jdNum;_log.push(`🟢${eventName}:${nickname}当前有${totalBeans}个京豆`);resolve([nickname,totalBeans])}else{throw err||data;}}catch(error){_log.push(`🔴${eventName}:${error}`);_desc.push(`🔴${eventName}`);resolve([])}})})}
// prettier-ignore
