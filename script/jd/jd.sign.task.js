...
function jdSignInfo(cookie) {
  const eventName = '【京东签到信息】';
  const option = getOption(
    'https://api.m.jd.com/client.action?functionId=signBeanIndex&body=%7B%22monitor_refer%22%3A%22%22%2C%22rnVersion%22%3A%223.9%22%2C%22fp%22%3A%22-1%22%2C%22shshshfp%22%3A%22-1%22%2C%22shshshfpa%22%3A%22-1%22%2C%22referUrl%22%3A%22-1%22%2C%22userAgent%22%3A%22-1%22%2C%22jda%22%3A%22-1%22%2C%22monitor_source%22%3A%22bean_m_bean_index%22%7D&appid=ld&client=android&osVersion=14.6',
    {
      Cookie: cookie,
  ...
  return new Promise((resolve, reject) => {
    $.get(option, (err, resp, data) => {
      try {
        if (resp.statusCode === 200 && JSON.parse(data).code === '0') {
          if (JSON.parse(data).data.status === '2') {
      ...
      try {
        if (resp.statusCode === 200 && JSON.parse(data).data) {
          if (JSON.parse(data).data.status === '1') {
            _beans += Number(JSON.parse(data).data.dailyAward.beanAward.beanCount) || 0;
            _log.push(`🟢${eventName}: 获得${JSON.parse(data).data.dailyAward.beanAward.beanCount}个京豆`);
            _desc.push(`🟢${eventName}`);
...
async function main(cookie) {
  ...
  const isSignIn = await jdSignInfo(cookie.cookie);
  if (isSignIn === false) {
    await jdSignIn(cookie.cookie);
    const [nickname, totalBeans] = await getUserInfo(cookie.cookie);
    $.subt = `${nickname}, 京豆: ${totalBeans}(+${_beans})`;
  } else {
    $.subt = `${cookie.nickname}, 今日已签到 ~`;
  }
}
