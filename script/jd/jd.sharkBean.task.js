...
async function doneTasks(cookie) {
  const eventName = '【浏览活动】';
  let s = 0;
  let i = 1;
  const tasks = await getTasks(cookie);
  if (tasks.length > 0) {
    for (const task of tasks) {
      const res = await browse(cookie, task.id, task.title);
      if (res) {
        s++;
      }
      if (i !== tasks.length) {
    ...
    _desc.push(`🟢${eventName} ${s}/${tasks.length}`);
  }
}
...
async function main(cookieObj) {
  ...
  if (_desc.length > 0) {
    ...
    $.subt = `${cookieObj.nickname}, Nothing to do ~`;
  }
}
