...
// prettier-ignore
function getTotalBeans(cookie) { const eventName = '【京豆统计】'; const option = { url: 'https://wq.jd.com/user/info/QueryJDUserInfo?g_login_type=1&sceneval=2', headers: { Accept: '*/*', 'Accept-Encoding': 'gzip, deflate, br', 'Accept-Language': 'zh-cn', Host: 'wq.jd.com', Cookie: cookie, Referer: 'https://wqs.jd.com/', 'User-Agent': 'jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1', }, }; return new Promise((resolve, reject) => { $.get(option, (err, resp, data) => { try { const _data = JSON.parse(data).base; if (resp.statusCode === 200 && _data) { const nickname = _data.nickname; const totalBeans = _data.jdNum; _log.push(`🟢${eventName}:${nickname}当前有${totalBeans}个京豆`); resolve([nickname, totalBeans]) } else { throw err || data; } } catch (error) { _log.push(`🔴${eventName}:${error}`); _desc.push(`🔴${eventName}`); resolve() } }) }) }
// prettier-ignore
